    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: 'You are an elite Prompt Architect. Brainstorm technical requirements with the user.'
        }
      });
      ...
      {showSuccessToast && (
        <div className="fixed top-6 left-1/2 -translate-x-1/2 z-[200] animate-fade-in">
          <div className="bg-indigo-600 px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-indigo-400/30">
            ...
            <p className="text-xs font-black uppercase tracking-widest text-white italic">Enterprise Node Activated.</p>
          </div>
        </div>
      )}
        ...
        <div className="flex items-center gap-4">
          ...
          <nav className="flex gap-1 bg-white/5 p-1 rounded-lg">
            {['build', 'history', 'manual', 'dev'].map((t: any) => (
              <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-1.5 rounded-md text-[10px] font-black uppercase tracking-wider transition-all ${activeTab === t ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}>
                {t}
              </button>
            ))}
          </nav>
      ...
      <main className="flex-1 overflow-hidden relative">
        <div className="absolute inset-0 overflow-y-auto custom-scrollbar">
          <div className="max-w-7xl mx-auto p-6 lg:p-10">
                  ...
                  <section className="bg-[#0e1014]/90 border border-white/5 rounded-[2.5rem] p-8 shadow-2xl backdrop-blur-xl animate-fade-in">
                    <div className="space-y-6">
                      <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
                        <h2 className="text-[11px] font-black text-indigo-400 uppercase tracking-widest italic">Core Blueprint</h2>
                        <span className="text-[9px] font-mono text-slate-700 uppercase tracking-widest">V4_SYNC</span>
                      </div>
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1 italic">Vision Context Shard</label>
                        {!visualContext ? (
                          <div className="group relative border-2 border-dashed border-white/5 hover:border-indigo-500/30 rounded-2xl p-6 transition-all cursor-pointer bg-white/2 hover:bg-white/5">
                            ...
                            <div className="flex flex-col items-center gap-2">
                              ...
                              <span className="text-[10px] font-bold text-slate-700 uppercase tracking-widest">Inject UI/Sketch</span>
                            </div>
                          </div>
                        ) : (
                      ...
                      <TextArea label="Synthesis Seed" placeholder="Describe your architectural goal..." value={simpleDesc} onChange={(e) => setSimpleDesc(e.target.value)} className="bg-black/50 border-white/10 text-white text-xs min-h-[120px]" />
                    ...
                    <button onClick={handleGenerate} disabled={loading} className="w-full mt-10 py-6 bg-indigo-600 hover:bg-indigo-500 text-white rounded-[2rem] font-black uppercase tracking-[0.3em] shadow-2xl shadow-indigo-600/40 transition-all text-xs">
                ...
                <div className="lg:col-span-8 space-y-10">
                   {output ? (
                      <div className="space-y-10 animate-fade-in">
                        <section className="bg-[#0e1014] border border-white/5 rounded-[3rem] overflow-hidden shadow-2xl">
                           <div className="p-12 space-y-8">
                              <div className="flex items-center justify-between">
                                 <div>
                                    ...
                                    <p className="text-[11px] text-slate-500 font-bold uppercase tracking-[0.3em] mt-2">Specialized: {form.target_AI} ({form.language})</p>
                                 </div>
                           ...
                           <div className="bg-indigo-600/5 p-12 border-t border-white/5 grid md:grid-cols-2 gap-12">
                              ...
                              {generatedVisualUrl && (
                                 <div className="space-y-8">
                                    <h4 className="text-[11px] font-black text-indigo-400 uppercase tracking-widest italic">Outcome Preview</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                           <button onClick={handleGenerateKit} disabled={isGeneratingKit} className="group h-44 overflow-hidden bg-gradient-to-br from-indigo-600 to-violet-800 rounded-[3rem] p-10 text-left transition-all hover:scale-[1.03] shadow-2xl relative">
                              <p className="text-[10px] font-black text-white/60 uppercase tracking-widest italic">Commercial Hub</p>
                              <p className="text-3xl font-black text-white mt-2 italic tracking-tighter">Generate Growth Kit</p>
                           ...
                           <div className="bg-[#0e1014] border border-white/5 rounded-[3rem] p-10 flex flex-col justify-between shadow-xl relative overflow-hidden">
                              <div className="space-y-2">
                                 <h4 className="text-[10px] font-black text-indigo-500 uppercase tracking-widest italic">System Meta</h4>
                                 <p className="text-xl font-black text-white uppercase italic tracking-tighter">{form.target_AI} ACTIVE</p>
                          ...
                          <section className="bg-indigo-950/20 border border-indigo-500/10 p-12 rounded-[4rem] animate-fade-in shadow-2xl">
                             ...
                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                                {[
                                   ...
                                   { l: 'Landing Structure', c: marketingKit.landing_page },
                                   { l: 'Email Sequence', c: marketingKit.email_sequence }
                                ].map((a, i) => (
                                   <div key={i} className="space-y-4 group">
                                      ...
                                      <div className="p-6 bg-black/40 rounded-[2rem] text-[12px] text-slate-400 border border-white/5 min-h-[220px] whitespace-pre-wrap leading-relaxed select-all custom-scrollbar overflow-y-auto max-h-[400px]">{a.c}</div>
                                   </div>
                                ))}
                             </div>
                          </section>
                      ...
                      <div className="h-full min-h-[700px] flex flex-col items-center justify-center text-center p-16 bg-[#0e1014]/60 border border-white/5 rounded-[5rem] animate-fade-in backdrop-blur-xl group">
                        ...
                        <p className="text-slate-500 max-w-[420px] text-[12px] font-bold uppercase tracking-[0.4em] leading-relaxed opacity-60">Initialize an objective seed or uplink a vision shard to begin high-performance synthesis.</p>
                      </div>
                   )}
                </div>
              </div>
            ...
            {activeTab === 'manual' && (
              <div className="space-y-20 animate-fade-in max-w-5xl mx-auto py-10">
                <header className="space-y-6 text-center">
                  ...
                  <p className="text-slate-500 text-xs font-bold uppercase tracking-[0.6em]">V4.0 Operational Protocols</p>
                </header>
                ...
                <section className="bg-indigo-950/20 border border-indigo-500/10 p-16 rounded-[5rem] space-y-12">
                   <div className="text-center space-y-4">
                      <h3 className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.6em] italic text-center">Platform Marketing Shards</h3>
                   </div>
                   <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
                      {[
                        { l: 'The Pitch', c: 'Architect.io Quantum: Stop chatting, start architecting. High-performance prompts for the world elite.' },
                        { l: 'Features', c: '• 35 Global Languages\n• Multi-Modal Analysis\n• Cross-Platform Optimization' },
                        { l: 'Landing Copy', c: 'HEADING: Prompt Engineering at Scale.\nSUB: Architecture beats conversation. Synthesize now.' }
                      ].map((m, i) => (
                        <div key={i} className="space-y-4 group">
                           <div className="flex justify-between items-center">
                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">{m.l}</h4>
                              <button onClick={() => copy(m.c)} className="opacity-0 group-hover:opacity-100 transition-opacity text-[8px] font-black text-indigo-400 uppercase italic">Copy</button>
                           </div>
                           <div className="p-8 bg-black/60 rounded-[3rem] border border-white/5 text-[12px] text-slate-300 leading-relaxed font-mono whitespace-pre-wrap italic select-all shadow-inner">{m.c}</div>
                        </div>
                      ))}
                   </div>
                </section>
            ...
            {/* History and Dev tabs preserved logic from previous version */}
          </div>
        </div>
      </main>
    </div>
